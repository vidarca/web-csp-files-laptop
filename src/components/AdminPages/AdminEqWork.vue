<template>
  <div id="admin-eq-work" class="position-relative" v-if="show">
    ...
    <section class="list-db w-auto" v-show="showList === true" ref="section0">
      ...
      <div v-if="listEmpty">
        <ul class="list-item w-100 row align-items-center justify-content-start" v-for="(integrante, index) in Object.values(dbWeb.Gerencia).reverse()" :key="integrante.gr_id" :id="integrante.gr_id" :data-id="`elem${index}`" data-transitioned="false" v-show="index < numElements + showIndex && index >= showIndex" ref="listElements">
          <li class="item-element text col-3 pl-1 pr-1">
            {{integrante.gr_nombre}}
          </li>
          <li class="item-element text col-3 pl-1 pr-1">
            {{integrante.gr_correo}}
          </li>
          <li class="item-element text col-3 pl-1 pr-1">
            {{integrante.gr_cargo}}
          </li>
    ...
    <section class="translate p-2" id="section1" v-show="showPrev" v-if="dbWeb.Gerencia !== undefined && selectIntegrante !== ''" ref="section1">
      <form class="form-class d-flex flex-column justify-content-center align-items-center">
        <div class="file-form">
          ...
          <!-- PREV IMAGENES -->
          <div  class="uploadCont flex-row align-items-center w-100 mt-2" v-show="selectIntegrante.imagen.nombre !== undefined" style="display: flex;">
            <div :class="['prev-container','d-flex', 'flex-column', 'align-items-center', 'justify-content-between', 'w-100', 'bg-success-light']">
              ...
              <div class="w-100 h-100 m-2">
                <div class="d-flex justify-content-center align-items-center" v-if="(dbImg[0] !== undefined || selectIntegrante.imagen.url !== '')">
                  <img :src="(dbImg[0] !== undefined && dbImg[0].url !== undefined)?dbImg[0].url:(selectIntegrante.imagen.url !== '')?selectIntegrante.imagen.url:''" width="360" height="260" style="min-height: 360px; min-width: 260px; ">
                </div>
                <div v-else class="w-100 h-100 d-flex flex-row align-items-center justify-content-center">
                  <p class="align-self-star ml-2" style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">{{selectIntegrante.imagen.nombre}}</p>
                </div>
                <!-- ICONO DE QUITAR -->
    ...
    <section class="translate p-2" id="section2" v-show="showCreate" ref="section2">
      <form class="form-class d-flex flex-column justify-content-center align-items-center">
        <div class="file-form">
          <!-- Inicio de la imagen -->
          <div class="text text-center">
          ...
          <!-- PREV IMAGENES -->
          <div  class="uploadCont flex-row align-items-center w-100 mt-2" v-show="nuevoIntegrante.imagen.nombre !== undefined" style="display: flex;">
            <div :class="['prev-container','d-flex', 'flex-column', 'align-items-center', 'justify-content-between', 'w-100', 'bg-success-light']">
              <!-- MUESTRA DE IMAGEN -->
              <div class="w-100 h-100 m-2">
                ...
                <div v-else class="w-100 h-100 d-flex flex-row align-items-center justify-content-center">
                  <p class="align-self-star ml-2" style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">{{nuevoIntegrante.imagen.nombre}}</p>
                </div>
                <!-- ICONO DE QUITAR -->
                <div class="veri-box w-100 mt-2 d-flex flex-row justify-content-center align-items-center">
...
export default {
    ...
    methods:{
      ...
      itemSelected(index){
        ...
        setTimeout(() => {
          ...
          this.selectIntegrante = {
            cargo: Object.values(this.dbWeb.Gerencia).reverse()[index].gr_cargo,
            id: Object.values(this.dbWeb.Gerencia).reverse()[index].gr_id,
            nombre: Object.values(this.dbWeb.Gerencia).reverse()[index].gr_nombre,
      ...
      deleteFile(index, val){
        if(val === 'crear'){
          this.nuevoIntegrante.imagen = '';
          this.files = {};
          this.resetDBValues(index);
        }else if(val === 'select'){
          this.selectIntegrante.imagen = '';
          this.files = {};
          this.resetDBValues(index);
        }
      },
      deleteCollection(value){
        if(value === 'crear'){
          this.nuevoIntegrante.correo = '';
          this.nuevoIntegrante.imagen = '';
          this.nuevoIntegrante.cargo = '';
          this.nuevoIntegrante.nombre = '';
          this.nuevoIntegrante.id = '';
        }else if(value === 'select'){
          this.selectIntegrante.correo = '';
          this.selectIntegrante.imagen = '';
          this.selectIntegrante.id = '';
          this.selectIntegrante.nombre = '';
          this.selectIntegrante.cargo = '';
        }
      ...
      submitCollection(value){
        ...
        if(value === 'crear'){
          let dataTransfer = {
            archivos: this.files,
            integrante: {
              nombre: this.nuevoIntegrante.nombre,
          ...
          let dataTransfer = {
            archivos: this.files,
            integrante: {
              nombre: this.selectIntegrante.nombre,
    ...
    watch:{
      ...
      successUpload(){
        if(this.deletingVal !== null && this.dbWeb.Gerencia !== undefined && Object.values(this.dbWeb.Gerencia).length - 1 === 0){
          this.listEmpty = false;
          let n = {
            0: 0,
            1: null,
